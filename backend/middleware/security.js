...
const removeUnsafeKeys = (value) => {
  ...
  for (const key of Object.keys(value)) {
    if (key === '__proto__' || key === 'constructor' || key === 'prototype') delete value[key];
...
export const securityHeaders = (req, res, next) => {
  ...
  res.set('Content-Security-Policy', [
    ...
    "frame-src https://accounts.google.com", "connect-src 'self' https://accounts.google.com https://oauth2.googleapis.com",
    "img-src 'self' data: https://lh3.googleusercontent.com", "style-src 'self' 'unsafe-inline'",
...
export const validateRequestBody = (req, res, next) => {
  if (stateChangingMethods.has(req.method) && req.path.startsWith('/api/') && !req.is('application/json')) {
