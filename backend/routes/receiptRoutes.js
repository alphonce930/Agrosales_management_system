...
router.post('/', async (req, res) => {
  try {
    ...
    if (!Number.isInteger(saleId) || saleId <= 0) {
      return res.status(400).json({ message: 'Select a sale before generating a receipt.' });
    }
    ...
    const sale = sales[0];
    const existing = await query('SELECT id FROM receipts WHERE sale_id = ? LIMIT 1', [sale.id]);
    let receiptId;
    if (existing.length) {
      receiptId = existing[0].id;
      if (notes) await query('UPDATE receipts SET notes = ? WHERE id = ?', [notes, receiptId]);
    } else {
      const receiptNumber = `RCT-${Date.now()}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`;
    ...
    const receipts = await query(`
      ...
      WHERE r.id = ?
    `, [receiptId]);
    return res.status(existing.length ? 200 : 201).json({ message: 'E-receipt generated successfully.', receipt: receipts[0] });
  } catch (error) {
    return res.status(error.status || 500).json({ message: error.message || 'Receipt generation failed.' });
  }
});
