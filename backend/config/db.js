...
const unavailableError = () => {
  const error = new Error(
    "Database is unavailable. Check the database connection and try again.",
  );
...
const fallbackQuery = async (sql, params = []) => {
  ...
  if (
    ...
    return fallbackStore.users.filter(
  ...
  if (
    normalized.startsWith("SELECT * FROM users WHERE email = ? OR username = ?")
  ) {
    const search = params[0] || "";
    return fallbackStore.users.filter(
      (user) => user.email === search || user.username === search,
  ...
  if (
    normalized.startsWith(
      "SELECT id, full_name, username, email, phone, location, profile_picture, auth_provider, role, status FROM users WHERE id = ?",
    )
  ) {
    const id = Number(params[0]);
    return fallbackStore.users.filter((user) => user.id === id);
  ...
  if (normalized.startsWith("INSERT INTO users")) {
    ...
    const newUser = {
      id: fallbackStore.users.length
        ? Math.max(...fallbackStore.users.map((u) => u.id)) + 1
        : 1,
      full_name: params[0],
      username: params[1],
      email: params[2],
  ...
  if (
    normalized.startsWith("UPDATE users SET role = ? AND status = ?") ||
    normalized.startsWith("UPDATE users SET role = ?, status = ?")
  ) {
    const user = fallbackStore.users.find(
      (entry) => entry.id === Number(params[2]),
    );
    if (user) {
      user.role = params[0];
  ...
  if (normalized.startsWith("UPDATE users SET google_id = ?")) {
    const user = fallbackStore.users.find(
      (entry) => entry.id === Number(params[4]),
    );
    if (user) {
  ...
  if (normalized.startsWith("INSERT INTO activity_logs")) {
    const newLog = {
      id: fallbackStore.activityLogs.length
        ? Math.max(...fallbackStore.activityLogs.map((l) => l.id)) + 1
        : 1,
      user_id: params[0],
      action: params[1],
  ...
  if (
    normalized.startsWith("SELECT c.*, COALESCE(creator.full_name") ||
    normalized.startsWith("SELECT c.*, c.initial_amount + COALESCE")
  ) {
    const customers = [...fallbackStore.customers].sort(
      (first, second) =>
  ...
  if (normalized.startsWith("INSERT INTO customers")) {
    const customer = {
      id: fallbackStore.customers.length
        ? Math.max(...fallbackStore.customers.map((entry) => entry.id)) + 1
        : 1,
      ...
      created_by_name: params[10]
        ? fallbackStore.users.find((user) => user.id === Number(params[10]))
  ...
  if (normalized.startsWith("SELECT * FROM customers WHERE id = ?")) {
    return fallbackStore.customers.filter(
      (customer) => customer.id === Number(params[0]),
    );
  }
  ...
  if (normalized.startsWith("DELETE FROM customers WHERE id = ?")) {
    ...
    fallbackStore.customers = fallbackStore.customers.filter(
  ...
  if (
    normalized.startsWith("SELECT p.*, c.name AS category_name FROM products p")
  ) {
    return [...fallbackStore.products].sort(
      (first, second) =>
        new Date(second.created_at) - new Date(first.created_at),
    );
  }
  ...
  if (normalized.startsWith("INSERT INTO products")) {
    const product = {
      id: fallbackStore.products.length
        ? Math.max(...fallbackStore.products.map((entry) => entry.id)) + 1
        : 1,
      ...
      pieces_per_box: params[8],
      minimum_stock: params[9],
      status: params[10],
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
      category_name: null,
...
export const query = async (sql, params = []) => {
  ...
  try {
    ...
    console.warn(
      `Database connection unavailable; retrying in ${databaseRetryDelay / 1000} seconds.`,
    );
    if (allowMemoryFallback) return fallbackQuery(sql, params);
    throw unavailableError();
  }
