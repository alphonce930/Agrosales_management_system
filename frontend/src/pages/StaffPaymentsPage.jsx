...
export default function StaffPaymentsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [customerRes, saleRes, paymentRes, receiptRes] =
          await Promise.all([
            api.get("/customers"),
            api.get("/sales"),
            api.get("/payments"),
  ...
  const customerSales = useMemo(() => {
    ...
    return sales.filter(
      (sale) => String(sale.customer_id) === String(form.customer_id),
    );
  ...
  useEffect(() => {
    if (
      ...
      (!form.sale_id ||
        !customerSales.some((sale) => String(sale.id) === String(form.sale_id)))
    ) {
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5">
        <div className="mb-4 flex items-center gap-2 text-lg font-semibold text-slate-900">
          <ReceiptText size={18} className="text-brand-deep" /> Record customer
          payment
        </div>
        ...
        >
          ...
          <select
            value={form.sale_id}
            onChange={(e) =>
              setForm((prev) => ({ ...prev, sale_id: e.target.value }))
            }
            className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 outline-none focus:border-brand-deep"
          ...
          <button
            disabled={saving}
            type="submit"
            className="rounded-xl bg-brand-deep px-4 py-2.5 font-medium text-white hover:bg-brand-green disabled:opacity-60"
          >
        ...
        <div className="card p-5">
            ...
            <button
              onClick={() =>
                openReceiptWindow({
                  ...
                  invoice: selectedReceipt.sale_number,
                  issued: formatDate(
                    selectedReceipt.payment_date || selectedReceipt.issued_at,
                  ),
                  ...
                  amountValue:
                    selectedReceipt.payment_amount ??
          ...
          <div className="mx-auto max-w-[420px] rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="text-center">
              ...
              <div className="mt-3 border-b border-dashed border-slate-300 pb-3 text-lg font-bold uppercase text-slate-900">
                Payment Receipt
              </div>
            </div>
